...
/*
只匹配出一条符合条件的，最近时间的未回调或未回调成功的预支付信息 
*/
function matchPreInfo(
                   v_userName             in varchar2,   --预支付用户账号
                   n_cpId                 in number,     --合作商ID标识
                   n_gameId               in number,     --游戏ID
                   n_gatewayId            in varchar2,     --区服ID
                   n_productId            in varchar2,   --产品标识
                   cur_result            out sys_refcursor
  )return integer;
/*
...
function updateUserName(
                         v_paymentId       in varchar2,--预支付订单号
                         v_realName        in varchar2--正式账号
                           )return integer;
...
begin 
  ...
  if(n_result =  E_CHARGE_DUPLICATE)then -- 订单号重复
     ...
     if(n_result = pkg_linekong_util.S_SUCCESS)then
        ...
        --查询预支付表的test_state值是否为9，如果为9，则把log_charge_common表的对应字段改成9 
...
/*
   将预支付表的试玩账号改成正式账号
  */
function updateUserName( v_paymentId       in varchar2,--预支付订单号
                         v_realName        in varchar2 --正式账号
                           )return integer is
   n_result          integer := pkg_linekong_util.S_SUCCESS;
   n_temp            integer;
