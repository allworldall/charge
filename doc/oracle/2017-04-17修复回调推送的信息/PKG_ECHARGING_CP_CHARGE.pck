...
function getPreInfo(v_payMentId           in varchar2,   --预支付ID
 ...
 begin
   ...
   if(n_val = 0)then
     ...
     open cur_result for
       select payment_id,user_name,cp_id,game_id,gateway_id,charge_money,charge_amount,product_id,attach_code,expand_info from log_pre_payment where payment_id = v_payMentId;
   end if;
   <<ExitError>>
   return n_result;
 end;
