...
public class MonitorInfoUpload implements Runnable{
    ...
    public void run() {
        if(timer >= MonitorConfig.getTimer()){//进行监控上报
            //监控时间超时
            MonitorConfig.uploadInfo(MonitorConfig.getInvokeUrl(), "unionProvider("+ Common.getLocalIp()+")", "警告-超时", message +"timeout:"+timer, null);
        }else{
            //匹配监控的错误码
            if(MonitorConfig.getResultCode().indexOf(result+";") >= 0){
                MonitorConfig.uploadInfo(MonitorConfig.getInvokeUrl(), "unionProvider", "警告-错误码", message ,result);
            }
        }
    }
}
